...
function addActiveEntity(id) {
	...
	var active_entity = new ActiveEntity(selected_entity.name, selected_entity.description,selected_entity.changeTypeDescription,selected_entity.changeRate,document.getElementById(id).value);
...
function initWorkspace() {
	var html = '<table border="1"><thead><tr><th colspan="2">Name</th><th colspan="2">Description</th><th colspan="2">InitialValue</th><th colspan="2">Type</th><th colspan="1">Rate</th><th colspan="1">Add</th></tr></thead><tbody>';
	Object.values(available_entities).forEach(entity => {
		var entityHtml = '<tr><td colspan="2">'+entity.name+'</td colspan="2"><td colspan="2">'+entity.description+'</td colspan="2"><td colspan="2"><input type="number" value=0 min=0 id="'+entity.id+'"></td><td colspan="2">'+entity.changeTypeDescription+'</td><td colspan="1">'+entity.changeRate+'</td><td colspan="1"><button onclick="addActiveEntity(\''+entity.id+'\')">Add</button></td></tr>';
		html+=entityHtml;
	});
...
function buildDashboard() {
	...
	if (Object.keys(activeEntities).length > 0) {
		 html = '<table border="1"><thead><tr><th colspan="2">Name</th><th colspan="2">Description</th><th colspan="2">InitialValue</th><th colspan="2">Type</th><th colspan="1">Rate</th><th colspan="2">Current Value</th><th colspan="1">Remove</th></tr></thead><tbody>';
		Object.values(activeEntities).forEach(entity => {
			entity.computeFor(duration);
			html+= '<tr><td colspan="2">'+entity.name+'</td><td colspan="2">'+entity.description+'</td><td colspan="2">'+entity.initialValue+'</td><td colspan="2">'+entity.changeTypeDescription+'</td><td colspan="1">'+entity.changeRate+'</td><td colspan="2">'+Math.round((entity.currentValue + Number.EPSILON) * 100) / 100+'</td><td colspan="1"><button onclick="removeActiveEntity(\''+entity.id+'\')">Remove</button></td></tr>';
		});
		html+= '</tbody></table>';
	} else {
